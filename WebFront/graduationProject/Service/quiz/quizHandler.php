...
function updateQuiz($param)
{
    ...
    $result = array();
    $con = initCon();
    $sql = "DELETE FROM quiz WHERE quizID = '" . $quizAllData['quiz']['quizID'] . "';DELETE FROM quiz_ask WHERE quizID = '" . $quizAllData['quiz']['quizID'] . "';DELETE FROM quiz_ask_elements WHERE quizID = '" . $quizAllData['quiz']['quizID'] . "';";
    if ($con->multi_query($sql)) {
        if (newQuiz($param)['code'] == '829') {
            $result['code'] = '829';
            $result['result'] = '成功';
        }
    } else {
...
function deleteQuiz($param)
{
    ...
    $result = array();
    $con = initCon();
//    $sql = "DELETE FROM quiz WHERE quizID = '" . $quizID . "';DELETE FROM quiz_ask WHERE quizID = '" . $quizID . "';DELETE FROM quiz_ask_elements WHERE quizID = '" . $quizID . "';";
